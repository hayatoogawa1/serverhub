...
- バージョン: 1.0
- 最終更新: 2026-09-04
- ステータス: **Phase 1 要件定義 確定**。MVP のスコープ・機能・非機能・業務ルール・
  概念データ・画面の要件を定義済み。後続フェーズで詳細化する項目（Phase 2〜4）と、
  一部の運用寄りの未決（[open-issues.md](open-issues.md) の S2 ほか）を残す。

> 未決事項・確定履歴は [open-issues.md](open-issues.md) で管理する。
> §7 の As-Is は一般的なインフラ運用の想定に基づく。実運用と差異があれば随時更新する。
...
- パスワード変更・リセットの画面は **MVP 対象外**（Q1 確定）。必要時は管理スクリプトで対応
...
対象は §4.2 の利用シーン。**As-Is（現行）は一般的なインフラ運用の想定**（表計算ファイル /
個人メモ中心）。実運用と差異があれば随時更新する。ToBe は ServerHub 導入後。
...
- **主シナリオ**:
  ...
  3. 数値・グラフ（Recharts、基本設計 [06-ui](../design/basic/06-ui.md) D-UI-01 で確定）で表示。各集計から対応する絞り込み一覧へ遷移できる。
...
- API パス・バージョニング（`/api/v1`）とレスポンスエンベロープ形式は **基本設計 [02-api](../design/basic/02-api.md) で確定**（Q2、2026-09-04）。
  本章では相対的な振る舞いを定義する。
...
- **エラー**: `401`。未知の enum 値（`environment` / `status`）・不正な `sort` / `order` → `400`。
  `size` が許可値外の場合は FR-SRV-01 と同じく既定値に丸める（`400` にしない）。
  ソート項目・方向・ページングの詳細は基本設計 [02-api](../design/basic/02-api.md) §2.5。
...
| FR-COM-05 グラフ表示 | ダッシュボードの可視化 | 環境別/ステータス別/タグ別の件数を棒グラフ等で。ライブラリは Recharts（確定、[06-ui](../design/basic/06-ui.md) D-UI-01） |
...
- 検証観点:
  ...
  | ページング | `page >= 0`（負数・非整数は `400`）。`size` は許可値 `10` / `20` / `50` / `100` のみ。許可値以外は**既定値に丸める**（上限は実質 100。基本設計 [02-api](../design/basic/02-api.md) §2.5 で確定） |
...
| SC-05 | サーバー登録 | FR-SRV-04, FR-TAG-01 / UC-06 | モーダル（F6 確定） |
| SC-06 | サーバー編集 | FR-SRV-05, FR-TAG-01 / UC-07 | モーダル（SC-05 と同形式、F6 確定） |
| SC-07 | メンテナンス履歴一覧 | FR-MNT-01 / UC-10 | ページ。対象サーバーで絞り込み + テーブル + ページネーション |
| SC-08 | メンテナンス履歴登録 | FR-MNT-02 / UC-11 | モーダル（F6 確定）。サーバー詳細 or 履歴一覧から起動 |
...
- 登録・編集（SC-05/06/08）は**モーダル** + 保存後の再取得（invalidate）で更新（F6、FR-COM-06）。
- **PC 専用**（F7 確定）。タブレット・スマホ向けの最適化はしない。想定解像度は 1280px 幅以上。
...
| RK-03 | As-Is 業務フロー（§7）がオーナーの実運用と乖離 | 要件のズレ | §7 は一般的想定と明記。実運用との差異が判明したら §7 と関連章を更新 |
