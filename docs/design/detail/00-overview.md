...
本書は Phase 3（詳細設計）着手にあたっての **叩き台**。位置づけ・入力・Phase 3 で扱う範囲・
残る未決事項（Q3）・文書構成の案をまとめ、進め方をオーナーと相談したうえで確定する
（本書自体、確定後にバージョンを 1.0 に上げる）。
...
機能単位で切る案。認証は他機能と依存が薄く独立、タグは単独の CRUD API を持たず
サーバー登録/編集の一部（FR-TAG-01）+ サジェスト（FR-TAG-02）のみのため、
**タグはサーバーの文書に同居**させる案としている（独立させるほどの分量がないため）。

| # | ドキュメント（案） | 内容（案） | 対応 FR |
|---|---|---|---|
| 00 | overview（本書） | 位置づけ・入力・進め方・Q3 | — |
| 01 | common | 共通 DTO 実体（`PageRequest`/`PageResponse`/`ApiError`/`FieldError`）、共通バリデーション実装パターン（ソートホワイトリスト・IP/ホスト名バリデータ選定）、**エラーコード正式化（Q3）** | 横断 |
| 02 | auth | ログイン/ログアウト/現在ユーザーの DTO、`UserDetailsService`、成功/失敗ハンドラの詳細 | FR-AUTH-01〜03 |
| 03 | server（タグ同居） | `Server`/`Tag` Entity、DAO・SQL、登録/編集/削除/検索の Request/Response DTO、タグ付け・サジェスト | FR-SRV-01〜06、FR-TAG-01/02 |
| 04 | maintenance | `MaintenanceHistory` Entity、DAO・SQL、一覧/登録の DTO | FR-MNT-01〜03 |
| 05 | dashboard | 集計 DAO・SQL（GROUP BY・上位 N 件+その他の実装方針）、Response DTO | FR-DASH-01 |

**進め方（案）**: Phase 2 と同じ「1 文書 = 1 ブランチ = 1 PR」。文書は 00→01→(02〜05 は依存が薄いので
どの順でも可、まず 02-auth or 03-server から)。01-common を先に固めることで、02 以降が
`ApiError`/エラーコード/共通 DTO を前提にできる。

**この案について確認したいこと**:

1. 文書の切り方（タグをサーバーに同居させる、の是非）
2. 01-common を最初に固める順序でよいか
3. 各文書の記述粒度（Entity のフィールド列挙・DAO メソッドシグネチャ・DTO 定義まで書く想定）が
   詳細設計として適切か（Phase 5 実装時にコードへ機械的に落とせる程度を目安にしている）
