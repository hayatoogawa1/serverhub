...
- バージョン: 0.5（ドラフト）
- 最終更新: 2026-09-04
- ステータス: 1〜9 章 + 10.1 セキュリティ（確定）。10.2〜10.5・11〜13 章は Phase 1 で順次追記。
...
- **処理・業務ルール**:
  ...
  - ソート可能項目（ホワイトリスト、§10.1.7）: ホスト名 / 環境区分 / ステータス / 更新日時 / 登録日時。
    （【要承認】この項目セットで良いか）
...
- **処理・業務ルール**（Q8）:
  - タグ名は前後空白を除去。空文字は無視。1〜（上限、【要承認】50 文字目安）文字。
  - 大文字小文字は区別する（`web` と `Web` は別タグ）。全角半角の変換はしない。
  - 同一サーバーに同名タグを重複付与しない（リクエスト内の重複は 1 件に集約）。
  - 既存タグは再利用（マスタ的な `tags` テーブルを引く / なければ作成）。どのサーバーからも参照されなくなったタグの掃除は MVP 対象外（【要承認】）。
...
- **入力**: 入力中の文字列（前方一致 or 部分一致、【要承認】）。
- **処理・業務ルール**: 既存タグ名から一致するものを返す。件数上限（例: 20）。
- **出力**: タグ名の配列。
- **エラー**: `401`。
...
- **処理・業務ルール**: 実施日の新しい順。対象サーバーが論理削除されていても履歴自体は表示する（【要承認】。削除サーバーの履歴を一覧に出すか）。
...
- **入力**（B4）:
  ...
  | 実施日 | ✅ | 日付（`YYYY-MM-DD`）。未来日を許すか【要承認】（既定案: 許容） |
...
- **処理・業務ルール**（すべて `deleted_at IS NULL` 対象）:
  ...
  - タグ別のサーバー数（上位 N 件 + それ以外、表示仕様は §13。【要承認】N 件数）
...
### 9.4 この章で追加された【要承認】事項

[open-issues.md](open-issues.md) に F1〜F5 として記録:

- F1: 一覧のソート可能項目セット（ホスト名 / 環境 / ステータス / 更新日時 / 登録日時）
- F2: タグ名の最大文字数（50 文字目安）と未参照タグの掃除（MVP 対象外案）
- F3: タグサジェストの一致方式（前方一致 or 部分一致）
- F4: メンテナンス履歴一覧に「論理削除済みサーバーの履歴」を表示するか
- F5: メンテナンス実施日に未来日を許容するか（既定案: 許容）、ダッシュボードのタグ別表示件数 N
