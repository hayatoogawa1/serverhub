...
- **バックアップ**: 本番は RDS の自動バックアップ / スナップショット（Phase 9 で方針確定）。
  開発 DB は Docker ボリューム（`make db-reset` で破棄・再作成できる前提）。
...
- **タグ / サーバー / 履歴**: 初期投入は必須ではない。デモ用のダミーデータを用意する場合は
  実在しない値にする（§10.1.18）。
...
- **初期セットアップ**:
  - DB スキーマは Flyway マイグレーションで構築（アプリ起動時に自動適用）。
  - ログインユーザーはシード SQL / 初期化スクリプトで投入（§12.5）。
