...
import { SERVER_SORT_KEYS, type ServerListApiParams, type ServerSortKey } from '@/types/server'

/**
 * サーバー一覧の状態（URL クエリと双方向同期、06-ui D-UI-04 / D-UI-08）。
 * URL は Stitch の短縮名（q / env / status / tags / sort / order / page / size）。
 * `page` は **1 始まり**（表示・共有向け）。API へは `toServerListApiParams` で 0 始まりへ変換する。
 */
...
export function toServerListApiParams(params: ServerListParams): ServerListApiParams {
  return {
