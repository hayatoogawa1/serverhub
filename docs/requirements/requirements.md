...
- バージョン: 0.2（ドラフト）
- 最終更新: 2026-09-04
- ステータス: 1〜6 章 + 10.1 セキュリティ のドラフト（レビュー待ち）。7〜9 章・10.2 以降は Phase 1 で順次追記。
...
- **方式**: Spring Security のフォームログイン + サーバーサイドセッション（`CLAUDE.md` §3、
  [ADR 0001](../adr/0001-backend-technology-versions.md)）。JWT は採用しない。
...
- **ブルートフォース対策**: MVP ではログイン失敗の記録のみ。連続失敗によるアカウントロック /
  レート制限は**将来対応**（open-issues S3）。社内・小規模・同一オリジンという前提でリスクを許容。
...
- **セッションタイムアウト**: アイドルタイムアウトを設定する（既定案: 30 分。open-issues S4 で確定）。
...
| 項目 | MVP | 方針 |
|---|---|---|
| 管理ユーザーとアプリ接続ユーザーの分離 | **要検討（open-issues S1）** | 現状は単一ユーザー（[ADR 0001](../adr/0001-backend-technology-versions.md)、docker-compose）。理想は「スーパーユーザー = 管理・マイグレーション」「アプリ用ユーザー = 必要最小権限」。Flyway の実行ユーザーとアプリ実行ユーザーの分離も論点 |
| アプリ DB ユーザーの権限最小化 | **設計必須 / 実装は S1 の判断次第** | アプリ用ユーザーは対象スキーマの `SELECT/INSERT/UPDATE/DELETE` のみ（`DROP`・スキーマ変更不可）。マイグレーションは別権限で実行 |
...
- **既知の論点（open-issues S7）**: 現在 `application.yml` は開発用の既定値
  （`SPRING_DATASOURCE_PASSWORD:changeme` 等、`.env.example` と同一のダミー値）を持つ。
  「`application.yml` に値を書かない」を厳密に適用するなら、既定値を除去して環境変数必須に
  する案がある。現状は「コミット済みダミーと同一・本番は環境変数で上書き」という整理。
