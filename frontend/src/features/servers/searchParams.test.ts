...
describe('toApiParams', () => {
  it('page を 0 始まりへ、all / 空はパラメータを省く', () => {
    expect(toApiParams(DEFAULT_SERVER_LIST_PARAMS)).toEqual({
      keyword: undefined,
  ...
  it('URL の page=1 は API の page=0', () => {
    expect(toApiParams({ ...DEFAULT_SERVER_LIST_PARAMS, page: 1 }).page).toBe(0)
    expect(toApiParams({ ...DEFAULT_SERVER_LIST_PARAMS, page: 5 }).page).toBe(4)
  })

  it('環境フィルタは environment として渡す（env ではない）', () => {
    const api = toApiParams({ ...DEFAULT_SERVER_LIST_PARAMS, env: 'production' })
