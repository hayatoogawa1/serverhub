...
- バージョン: 0.4（ドラフト）
- 最終更新: 2026-09-04
- ステータス: 1〜8 章 + 10.1 セキュリティ（確定）。9 章・10.2〜10.5 は Phase 1 で順次追記。
...
| | As-Is | ToBe |
|---|---|---|
| 手順 | 台帳の該当行を編集。誰がいつ変えたか残らない。同時編集で上書き事故のリスク | 編集画面で変更 → 保存。`updated_at` が更新される。同時編集は競合として検知（【要承認】排他制御方式、open-issues Q7） |
...
- **主シナリオ**:
  ...
  3. 楽観的ロック（【要承認】open-issues Q7）で他者による更新がないか確認し、更新して `updated_at` を更新。タグの差分を反映。
- **例外**:
  - E1 入力不正: `400`。
  - E2 ホスト名重複: `409`。
...
- **業務ルール**: 同一サーバーに同名タグを重複付与しない。タグ名は前後空白除去・大文字小文字の扱いは §11 で確定（【要承認】）。
...
> 🚧 Phase 1 で追記（機能一覧 `FR-*` + 機能別詳細 + G↔FR↔UC トレーサビリティ表）。
> 各機能は §8 のユースケース（UC-01〜UC-13）を実装レベルに落としたもの。
...
| 2026-09-04 | 0.4 | 7 章（業務フロー: As-Is/ToBe と課題対応）、8 章（ユースケース UC-01〜UC-13、主要 UC 記述、共通例外、G↔UC トレーサビリティ）を追加。10.1.9 の情報分類を B3/B4 と整合。未決 Q7（排他制御）/ Q8（タグ名正規化）を追加 |
